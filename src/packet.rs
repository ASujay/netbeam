    ...
    pub fn encode(&self) -> Vec<u8> {
        match self {
            ...
            DiscoveryPacket::Info {
                ...
                let mut info_packet = Vec::<u8>::with_capacity(14 + hostname.len());
                ...
                info_packet.extend_from_slice(hostname);
                info_packet
            }
        }
    }
    ...
    pub fn decode(buf: &[u8]) -> Option<Self> {
        ...
        match magic {
            ...
            INFO_PACKET_IDENTIFIER => {
                ...
                let hostname = std::str::from_utf8(buf.get(14..)?).ok()?.to_owned();
...
mod test {
    ...
    fn create_info_packet_buffer(port: u16, request_id: u64, hostname: String) -> Vec<u8> {
    ...
    #[test]
    fn decode_info() {
        let buf = create_info_packet_buffer(16420, 16420, String::from("Aniket Sujay PC"));
        assert_eq!(
            DiscoveryPacket::decode(buf.as_slice()),
            Some(DiscoveryPacket::Info {
                port: 16420,
                request_id: 16420,
                hostname: String::from("Aniket Sujay PC"),
            })
        );
    }
    ...
    #[test]
    fn decode_info_with_empty_hostname() {
        let buf = create_info_packet_buffer(16420, 42, String::new());

        assert_eq!(
            DiscoveryPacket::decode(&buf),
            Some(DiscoveryPacket::Info {
                hostname: String::new(),
                port: 16420,
                request_id: 42,
    ...
    #[test]
    fn decode_info_with_invalid_utf8_hostname() {
        let mut buf = create_info_packet_buffer(16420, 42, String::new());
        buf.push(0xff);
    ...
    #[test]
    fn round_trip_info_with_unicode_hostname() {
        let packet = DiscoveryPacket::Info {
            hostname: String::from("अनिकेत-MacBook"),
            port: 16420,
            request_id: 42,
        };
        let encoded = packet.encode();

        assert_eq!(DiscoveryPacket::decode(&encoded), Some(packet));
    }
}
